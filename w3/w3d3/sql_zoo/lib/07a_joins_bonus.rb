...
def song_title_counts
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      tracks.song, COUNT(DISTINCT(albums.asin)) as num_times
    FROM
      tracks
    JOIN
      albums ON tracks.album = albums.asin
...
def best_value
  ...
  execute(<<-SQL)
    ...
    HAVING
      albums.price / COUNT(tracks.song) < .50
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      styles.style, SUM(DISTINCT(albums.price)) / num_tracks_per_album.acount
    FROM
      albums
    JOIN
      (
        ...
        GROUP BY
          albums.asin
      ) AS num_tracks_per_album ON albums.asin = num_tracks_per_album.atitle
    JOIN
      styles ON num_tracks_per_album.atitle = styles.album
    WHERE
      num_tracks_per_album.acount IS NOT NULL
    GROUP BY
      styles.style, num_tracks_per_album.acount
    ORDER BY
      SUM(DISTINCT(albums.price)) / num_tracks_per_album.acount DESC
  SQL
end
